...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tbodyScore = document.querySelector("tb");
  // 성적들 데이터를 저장할 배열을 선언
  ...
  const scoreCheck = () => {
    ...
    for (let i = 0; i < inputScores.length; i++) {
       ...
       * 1번 조건문이 실행되지 않았으며
       * 2번 조건문 검사를 하고, 2번 조건문이 실행 = 점수 < 0 , 점수 > 100 인 경우
       * "점수 범위..."alert 이 뜰 것이고
       * 실행은 input.focus() 로 점프한다
       ...
       *    input.focus(), return false가 실행되어
       * 검사 과정을 즉시 중단한다
  ...
  // 어딘가(scores)에 저장된 데이터를 lcoalStorage 에 보관하기
  const scoreSave = () => {
    if (scores) {
  ...
  const scorePrint = () => {
    ...
    for (let i = 0; i < scores.length; i++) {
      ...
      td.textContent = scores.sc_avg;
      tr.appendChild(td);

      // table 의tbody 에 이미 추가된 모든 tr, td 요소 제거
